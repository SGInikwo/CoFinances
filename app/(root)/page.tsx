...
const Home = async ({ searchParams }: PageProps) => {
  ...
  const [transactionList, allSummaries, monthList, transactionSummary] =
    await Promise.all([
  ...
  monthlyBalance = transactionSummary?.monthlyBalance
    ? Number(transactionSummary.monthlyBalance)
    : 0;
  monthlyExpenses = transactionSummary?.monthlyExpenses
    ? Number(transactionSummary.monthlyExpenses)
    : 0;
  monthlySavings = transactionSummary?.monthlySavings
    ? Number(transactionSummary.monthlySavings)
    : 0;

  earliestBalance = transactionSummary?.earliestBalance
    ? Number(transactionSummary.earliestBalance)
    : 0;
  earliestExpenses = transactionSummary?.earliestExpenses
    ? Number(transactionSummary.earliestExpenses)
    : 0;
  earliestSavings = transactionSummary?.earliestSavings
    ? Number(transactionSummary.earliestSavings)
    : 0;
        ...
        <div className="flex gap-1 w-full max-md:flex-col">
          ...
          <div className="hidden">
            <CurrentBalanceBox
              ...
              totalCurrentBalance={100050.0}
              totalPreviousBalance={500.2}
              totalTransactions={1}
              user_currency={loggedIn?.currency}
            />
          </div>
